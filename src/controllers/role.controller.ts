...
export class RoleController extends BaseController<RoleService> {
    ...
    public validateUniqueRoleName = async ( req: Request, res: Response ): Promise<unknown> => {
        try {
            const { roleName } = req.query

            const data = await this._service.findOneRoleByName( String( roleName ).toUpperCase() )

            if ( data ) return this._httpResponse.Ok( res, { isUnique: false } )

            return this._httpResponse.Ok( res, { isUnique: true } )
        } catch ( error ) {
            console.log( red( `Error in RoleController.validateUniqueRoleName: ` ), error )
            return this._httpResponse.InternalServerError( res, error )
        }
    }
