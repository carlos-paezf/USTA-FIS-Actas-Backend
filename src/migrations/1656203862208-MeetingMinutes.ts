...
export class MeetingMinutes1656203862208 implements MigrationInterface {
    name = 'MeetingMinutes1656203862208'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`meeting_minutes\` (\`id\` varchar(36) NOT NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, \`number_meeting_minutes\` int NOT NULL, \`is_regular\` tinyint NOT NULL DEFAULT 1, \`organism_committee_area_program\` varchar(255) NOT NULL, \`meeting_place\` varchar(255) NOT NULL, \`meeting_date\` timestamp NOT NULL, \`start_time\` varchar(255) NOT NULL, \`ending_time\` varchar(255) NOT NULL, \`produced_by_id\` varchar(36) NULL, \`reviewed_by_id\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`activity\` (\`id\` varchar(36) NOT NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, \`name_activity\` varchar(255) NOT NULL, \`activity_date\` timestamp NOT NULL, \`fulfillment\` enum ('Yes', 'No', 'In Process') NOT NULL, \`meeting_minutes_id\` varchar(36) NULL, \`responsible_user_id\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
